...
(function () {
  ...
  function computeToneScores(core, centreHz) {
    ...
    // NOTE: an earlier revision moved this ramp to -3.0 to stop a short
    // high-tone syllable collecting the rising bonus. It was reverted: the
    // TEXTBOOK rising reference only troughs at -3.0 itself, so the stricter
    // ramp punished learners for matching the very shape the app draws as their
    // target (the reference contour fell from 100% to 81%). Trough depth alone
    // cannot separate the two cases; the end-drop cue below does it without
    // penalising a correct rising tone.
