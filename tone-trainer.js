...
(function () {
  ...
  const SEARCH_LO = -14;             // period search band around the calibrated centre,
  const SEARCH_HI = 10;              // in semitones. Constraining the SEARCH is what
  ...
  const OCTAVE_SNAP_ST = 5.5;        // deviation from local context that triggers an octave snap
  ...
  function resolveOctavePath(frames, centreHz) {
    ...
    const nodes = frames.map(f => {
      ...
      return f.cands.map(c => {
        ...
        return { hz: c.hz, s: sv, emit: OCT_W_NSDF * (1 - c.v) + OCT_W_BAND * outside };
      });
    });
    const idx = [];
  ...
  function createToneCapture() {
    ...
    async function openMic() {
      ...
      // Window length is chosen to hold roughly ANALYSIS_PERIODS cycles of THIS
      // speaker's voice. A fixed window cannot serve both: 23ms holds 5 periods
      // of a 220Hz woman but only 2.3 of a 100Hz man, and MPM on 2.3 periods
      // locks onto subharmonics (measured: 23-45Hz reported for a ~110Hz voice).
      const hintHz = opts.centreHint > 0 ? opts.centreHint : 170;
      const want = ANALYSIS_PERIODS * sampleRate / hintHz;
      winLen = want <= 1024 ? 1024 : (want <= 2048 ? 2048 : 4096);
      if (!tracker || tracker.winLen !== winLen) tracker = makeTracker(winLen);
      if (!bufWin || bufWin.length !== winLen) bufWin = new Float32Array(winLen);
      // Search band. Constraining the period search to a plausible range around
      // the speaker's centre is what removes the whole class of x2/x3 harmonic
      // errors, rather than trying to repair them afterwards.
      if (opts.centreHint > 0) {
        searchMinHz = Math.max(F0_MIN, opts.centreHint * Math.pow(2, SEARCH_LO / 12));
        searchMaxHz = Math.min(F0_MAX, opts.centreHint * Math.pow(2, SEARCH_HI / 12));
      } else {
        searchMinHz = F0_MIN; searchMaxHz = F0_MAX;
      }

      const cap = Math.ceil(sampleRate * (MAX_CAPTURE_MS + 500) / 1000);
    ...
    function updateLiveVad(f) {
      ...
      if (!inSpeech) {
        ...
        if (!loud && (f.t - lastLoudT) > SPEECH_END_MS) {
      ...
      // Timeouts.
      if (confirmedSpeech && !inSpeech && (f.t - lastLoudT) > AUTO_STOP_SILENCE_MS) { stop('silence'); return; }
      if (!confirmedSpeech && f.t > NO_SPEECH_TIMEOUT_MS) { stop('nospeech'); return; }
      if (f.t > MAX_CAPTURE_MS) { stop('timeout'); return; }
    }
    ...
    function stop(reason) {
      ...
      if (opts.onEnd) {
        try { opts.onEnd({ frames: frames.slice(), thresholdRms, reason: reason || 'manual' }); } catch (e) {}
      }
    }
  ...
  function extractContour(frames, liveThreshold, centreHint) {
    ...
    // 0. Re-solve the octave choice with the whole word in hand. The live pass
    //    had to commit frame by frame; here we can see that (say) a halved
    ...
    let best = runs[0];
    for (const r of runs) if (r.count > best.count) best = r;
    ...
    let lo = best.start, hi = best.end;
    while (lo > 0) {
      const f = frames[lo - 1];
      if (f.hz > 0 && f.rms >= extGate && (frames[best.start].t - f.t) <= EXTEND_MS) lo--;
      else break;
    }
    while (hi < frames.length - 1) {
      const f = frames[hi + 1];
      if (f.hz > 0 && f.rms >= extGate && (f.t - frames[best.end].t) <= EXTEND_MS) hi++;
      else break;
    }
    ...
    // 6. Centred median smoothing, then the de-spiker.
  ...
  function trimTerminalCreak(pts) {
  ...
  function cleanCaptureCore(realPoints, thresholdRms) {
    ...
    return extractContour(frames, thresholdRms);
  }
  ...
  function computeToneScores(core, centreHz) {
    ...
    // A Thai HIGH tone also climbs — it starts slightly below centre and rises
    // to +4..+6. The difference is the TROUGH: a rising tone dips to -4..-9
    // first, a high tone barely dips below centre at all. Without this factor
    // every native high tone in the corpus was read as rising.
    const troughFactor = 0.15 + 0.85 * clamp01((-minV - 1.5) / 3.0);
    if (net >= 1.5 && maxV >= -0.3) {
      sc.rising = troughFactor * clamp01((net - 1.0) / 3.5)                 // upward movement
    ...
    sc.high = clamp01((mean - 0.3) / 3)                      // height above centre
            ...
            + clamp01(riseToPeak / 3) * 0.30                 // climbed INTO the peak,
              ...
              * clamp01((minV + 2.5) / 2.0);                 // ...but only when the voice
                                                             // never dipped deep first. A
                                                             // RISING tone also climbs into
                                                             // its peak, from a low trough,
                                                             // and must not collect this.
    ...
    const levelHigh = (mean >= -0.4 && minV >= -1.6 && dd < 1.6 && drop < 1.5 &&
                       maxV >= 0 && net < 1.5);
    ...
    return { sc, m: { first, last, mean, net, minV, maxV, maxAt, minAt, range, drop,
                      ...
                      dd, ddStartAt, ddEndAt, du, duStartAt, duEndAt,
  ...
  function onCalCaptureEnd(result) {
    ...
    const cleaned = extractContour(result.frames, result.thresholdRms);
