...
(function () {
  ...
  function trimTerminalCreak(pts) {
    ...
    // Measure the candidate tail FIRST, then decide whether it is creak at all.
    let cut = 0;
    for (let i = pts.length - 1; i > MIN_CORE_POINTS - 1; i--) {
      const p = pts[i];
      const quiet = (p.rms || 0) < CREAK_RMS_FRAC * p90;
      const low = (12 * Math.log2(p.hz / bodyHz)) < -CREAK_DROP_ST;
      if (quiet && low) cut++; else break;
    }
    if (cut === 0) return pts;

    // THE KEY TEST: how much of the word is this tail? A glottal release is
    // brief — it lives in the last fifth or so. A FALLING tone's descent runs
    // across most of the syllable, and it is also quiet (the voice fades as it
    // drops) and far below the body, so it satisfies both of the tests above.
    // Trimming it removed the bottom of the fall and turned falling tones into
    // high ones. If the quiet, low tail is long, it IS the tone: leave it alone.
    // KNOWN LIMIT: for two of the nine test speakers this test is not enough.
    // Their falling tones end in a plunge that is quiet, far below the body, AND
    // as brief as a glottal release, so no energy or duration rule separates the
    // two. Those words are read as high. Distinguishing them needs the onset —
    // a falling tone starts high, a high tone climbs to its peak first — which
    // is a scoring change rather than a trimming one.
    if (cut > pts.length * CREAK_MAX_FRAC) return pts;

    return pts.slice(0, pts.length - cut);
  }
  ...
  function computeToneScores(core, centreHz) {
    ...
    const riseSize = 0.15 + 0.85 * clamp01((du - 4.0) / 5.0);
    const riseLate = 0.20 + 0.80 * clamp01((duStartAt - 0.05) / 0.20);
