...
(function () {
  ...
  var TTD = (function () {
    ...
    function segment(frames, utt, N) {
      ...
      if (natural > 0 && Math.abs(natural - N) > Math.max(1, Math.round(N * 0.4))) {
        return { failed: true, found: natural, want: N, natural: true };
      }

      var lo = 0.002, hi = 0.9, nuclei = null, found = 0;
      for (var it = 0; it < 45; it++) {
        var mid = (lo + hi) / 2, pk = peaksAt(mid);
        found = pk.length;
        if (found === N) { nuclei = pk; break; }
        if (found > N) lo = mid; else hi = mid;
      }
      if (!nuclei) return { failed: true, found: found, want: N };

      var bounds = [];
      for (var a2 = 0; a2 < N - 1; a2++) {
        var best = nuclei[a2] + 1, bv = Infinity;
        for (var i4 = nuclei[a2] + 1; i4 < nuclei[a2 + 1]; i4++) {
          if (eS[i4] < bv) { bv = eS[i4]; best = i4; }
        }
        bounds.push(best);
      }
      ...
      return { syls: syls, nuclei: nuclei, t0: t0, n: n };
    }
    ...
    function analyse(frames, liveThreshold, centreHz, twister) {
      ...
      for (var i = 0; i < N; i++) {
        ...
        var best = { percent: 0, tone: null, isTarget: false };
        for (var a = 0; a < accepted.length; a++) {
          var r = (core.length >= 4)
            ? d.scoreAttempt(core, centre, accepted[a])
            : { percent: 0, tone: null, isTarget: false };
          if (r.percent > best.percent) best = r;
        }
        if (best.isTarget) hits++;
        sum += best.percent;
        results.push({
          index: i, percent: best.percent, heard: best.tone, isTarget: best.isTarget,
          startMs: seg.syls[i].startMs, endMs: seg.syls[i].endMs, points: cond
        });
      }
      return {
    ...
    /* VAD policy for sentences. speechEndMs 700 comes from measurement: the
       longest silent gap observed INSIDE a single native reading was 490ms, and
       the engine's 320ms default truncated real sentences after one or two
       words. minCaptureMs scales with the sentence so a mid-sentence pause
       always falls inside the floor. */
    function vadFor(twister) {
      var n = twister.syllables.length;
      return {
        multiSegment: true,
        speechEndMs: 700,
        autoStopSilenceMs: 900,
        minCaptureMs: Math.min(6000, 500 + n * 200),
        maxCaptureMs: Math.min(15000, 3000 + n * 1200),
        noSpeechMs: 6000
      };
    }
  ...
  function openTwister(tw) {
    ...
    resetAudio(tw);
    clearResult();
    setStatus('Tap the microphone and say the whole sentence.');
    setMode('practice');
  ...
  function startRecording() {
    ...
    setStatus('Listening\u2026 say the whole sentence.');
