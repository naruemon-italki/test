...
(function () {
  ...
  var TTD = (function () {
    ...
    function analyse(frames, liveThreshold, centreHz, twister) {
      ...
      /* Connected speech compresses how far the pitch travels within a
         syllable. Measured on a native reading, a clean rising tone spanned
         9.5 semitones where the tone model, tuned on ISOLATED words, expects
         11-16 — so a correct rising tone lands in the "high" band and is marked
         wrong. Expanding the excursion before scoring puts it back in range.

         Applied ONLY to rising and falling. Those are judged by how far the
         pitch travels, which is exactly what compression shrinks. Mid, low and
         high are judged by HEIGHT, and stretching a height about the centre
         distorts the very thing being measured — applying this to all five
         tones measured 68.4% but cost two mid/low syllables to buy two others,
         whereas contour-only gains the same two with no losses. Flat from 0.6
         to 0.9, so the exact value is not critical. */
      var CONTOUR_EXPAND = 0.80;
      ...
      for (var i = 0; i < N; i++) {
        ...
        var core = cond.map(function (p) {
          ...
          if (tones[i] === 'rising' || tones[i] === 'falling') v2 /= CONTOUR_EXPAND;
